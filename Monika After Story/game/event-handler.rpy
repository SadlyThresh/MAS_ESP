...
label call_next_event:
    ...
    if event_label and renpy.has_label(event_label):
        ...
        if (
            ...
            if renpy.windows:
                $ display_notif(m_name, mas_win_notif_quips, "Alerta de Temas")
            else:
                $ display_notif(m_name, mas_other_notif_quips, "Alerta de Temas")
...
label prompt_menu:
    ...
    if store.mas_globals.in_idle_mode:
        ...
        if cb_label is not None:
            call expression cb_label from _call_expression_7
    ...
    python:
        ...
        if len(unseen_event_labels) > 0 and persistent._mas_unsee_unseen:
            ...
            mas_setEVLPropValues(
                "mas_show_unseen",
                prompt="Me encantaría ver 'Texto no visto' ([unseen_num]) de nuevo"
            )
        else:
...
label mas_bookmarks_unbookmark(bookmarks_items):
    ...
    if len(bookmarks_items) > 1:
        ...
        $ renpy.say(m, "Sólo selecciona el marcador si estás seguro de que quieres quitarlo.", interact=False)

    call screen mas_check_scrollable_menu(bookmarks_items, mas_ui.SCROLLABLE_MENU_TXT_MEDIUM_AREA, mas_ui.SCROLLABLE_MENU_XALIGN, selected_button_prompt="Quitar seleccionados")
