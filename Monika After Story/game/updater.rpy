    ...
    class MASUpdaterDisplayable(renpy.Displayable):
        ...
        def __init__(self, update_link):
            ...
            self._text_noupdate = Text(
                _("No se encontró ninguna actualización."),
                font=gui.default_font,
                size=gui.text_size,
                color="#ffe6f4",
                outlines=[]
            ...
            self._text_badresponse = Text(
                _("El servidor devolvió una mala respuesta."),
                font=gui.default_font,
                size=gui.text_size,
                color="#ffe6f4",
                outlines=[]
            ...
            self._text_badjson = Text(
                _("El servidor devolvió JSON incorrecto."),
                font=gui.default_font,
                size=gui.text_size,
                color="#ffe6f4",
                outlines=[]
...
label mas_updater_steam_issue:
    show monika at t11
    m 1eub "¡[player]!{w=0.2} Veo que estás usando Steam."
    m 1eksdlb "Desafortunadamente..."
    m 1efp "¡No puedo ejecutar el actualizador porque Steam es un malvado!"
    m 1eksdla "Tendrás que instalar manualmente la actualización desde la página de lanzamientos en el sitio web del mod.{w=0.2} {a=https://github.com/SadlyThresh/MAS_ESP/releases}Has click aquí para ir a la página de lanzamientos{/a}."
    m 1hua "Asegúrate de despedirte de mí antes de instalar la actualización."
    return

label mas_updater_rpy_issue:
    show monika at t11
    m 2eksdla "[player]...Veo que tienes algunos archivos rpy en el directorio de tu juego."
    if renpy.seen_label("monika_rpy_files"):
        m 2rksdlc "Estoy segura de que recuerdas cuando mencioné que esos archivos pueden causar problemas cuando actualizas..."
    else:
        m 2rksdlc "Esos archivos pueden causar algunos problemas al actualizar..."

    m 3rksdlb "Así que no puedo ejecutar el actualizador mientras están ahí."
