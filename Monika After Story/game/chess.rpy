...
define mas_chess.CHESS_PROMPT_FORMAT = "{0} | {1} | Turno: {2} | Tú: {3}"
    ...
    if not renpy.seen_label("mas_chess_save_selected"):
        ...
        if quicksaved_file is None:
            ...
            if _return == mas_chess.CHESS_GAME_CONT:
                python:
                    try:
                        if os.access(quicksaved_filename_clean, os.R_OK):
                            ...
                            store.mas_utils.writelog("No se ha podido acceder al archivo rápido.\n")
                            quicksaved_file = None

                    except Exception as e:
                        store.mas_utils.writelog("ARCHIVO RÁPIDO: {0}\n".format(e))
                        quicksaved_file = None
                ...
                if quicksaved_file is None:
                    ...
                    m 2wfw "¡[player]!"
                    m 2wfx "Has vuelto a quitar el guardado."
                    pause 0.7
        ...
        # otherwise we are in good hands
            ...
            if failed_to_load_save:
                ...
                # cheat
                m 1eua "Todavía tenemos un juego inacabado en curso."
...
label mas_chess_remenu:
    python:
        menu_contents = {
            "gamemode_select": {
                "options": [
                    ("Ajedrez normal", False, False, not do_really_bad_chess),
                    ("Ajedrez aleatorio", True, False, do_really_bad_chess)
                ],
                "final_items": [
            ...
            "ruleset_select": {
                "options": [
                    ("Reglas casuales", True, False, casual_rules),
            ...
            "mode_select": {
                "options": [
                    ("Practica", True, False, practice_mode),
            ...
            "color_select": {
                "options": [
                    ...
                    ("¡Saquemos a suertes!", 0, False, is_player_white is 0) #Is check here specifically for states
                ],
                "final_items": [
                    ("Modo de juego", "gamemode_select", False, False, 20),
                    ("Reglas", "ruleset_select", False, False, 0),
    ...
    $ renpy.say(m, "¿Cómo te gustaría jugar?{0}".format("{fast}" if loopback else ""), interact=False)
    ...
    if _return == -1:
        show monika at t11
        m 1ekc "...Muy bien, [player].{w=0.3} Tenía muchas ganas de jugar contigo."
        m 1eka "Pero ya jugaremos en otro momento, ¿vale?"
        return
    ...
    #We changed a rule
        ...
        #Practice/Play mode
            ...
            if _return is 0:
                show monika at t11
                m 1eua "Si jugamos con reglas casuales, no contaremos los empates como si lo fueran.{w=0.2} {nw}"
                extend 3eub "Básicamente, el jugador que no está atrapado es declarado ganador."
...
label mas_chess_start_chess:
    ...
    if is_monika_winner:
        ...
        if is_surrender:
            if num_turns < 5:
                ...
                m 3ekb "Realmente significa mucho para mí que sigas jugando si lo haces~"
    ...
    #Stalemate
        ...
        if new_pgn_game.headers.get("DrawRequested"):
            ...
            m 1eka "Parece que estamos en un punto muerto."
            $ line_start = "Pero el lado bueno es que"

        if not persistent.ever_won.get("chess"):
            m 3hub "[line_start], estás cada vez más cerca de vencerme, [player]~"

        else:
            m 1hua "Buen trabajo para llegar hasta aquí, [player]~"
    ...
    #Player wins
        ...
        if practice_mode:
            ...
            if not undo_count:
                ...
                m 1hua "Solo has deshecho una vez. {w=0.2}{nw}"
                extend 3hub "¡Buen trabajo!"

            elif undo_count <= 5:
                m 1hua "Sólo has deshecho [undo_count] veces, buen trabajo."

            elif undo_count <= 10:
                m 1eua "[undo_count] desechos, no está nada mal. Si seguimos practicando juntos, estoy segura de que podemos bajar eso~"
...
label mas_chess_draw_lots(begin=True):
    show monika at t11
    $ drew_lots = True
    $ lets_begin = "{w=0.2} Empecemos." if begin else ""

    if random.randint(0, 1) == 0:
        $ is_player_white = chess.WHITE
        m 2eub "Oh mira, ¡he sacado las negras![lets_begin]"
    else:
        $ is_player_white = chess.BLACK
        m 2eub "Oh mira, ¡he sacado las blancas![lets_begin]"
    return
...
label mas_chess_savegame(silent=False, allow_return=True):
    ...
    # otherwise ask for name
        ...
        python:
            ...
            while len(save_name) == 0:
                save_name = mas_input(
                    "Introduzca un nombre para este juego:",
        ...
        if is_file_exist:
            ...
            m "¿Debería sobreescribirlo?{nw}"
            $ _history_list.pop()
            menu:
                m "¿Debería sobreescribirlo?{fast}"
                "Sí.":
                    pass
    ...
    if not silent:
        ...
        if not renpy.seen_label("mas_chess_savegame.pgn_explain"):
            ...
            m 3eub "¡Ya sea que ganes, pierdas, te rindas o empates, siempre hay algo que podrías haber hecho mejor, así que cargar estos juegos puede ayudarte a mejorar!"
...
label mas_chess_locked_no_play:
    ...
    m 1rsc "Ahora mismo no tengo ganas de jugar al ajedrez."
    return
...
label mas_chess_cannot_work_embarrassing:
    ...
    show monika at t11
    m 1rksdla "..."
    m 3hksdlb "Bueno, eso es vergonzoso, parece que no puedo hacer que el ajedrez funcione en tu sistema..."
...
label mas_chess_dlg_game_monika_wins_often:
    ...
    m 1eua "Volvamos a jugar pronto, ¿de acuerdo?"
...
label mas_chess_dlg_game_monika_wins_sometimes:
    ...
    if persistent._mas_chess_difficulty != (0, 1):
        m 3eua "Hasta entonces, intentaré ser un poco más suave contigo."
    return
    ...
    if pgn_files:
        ...
        if game_count > 1:
            if renpy.seen_label("mas_chess_save_multi_dlg"):
                $ pick_text = _("Todavía tienes que elegir un juego para mantener.")

            else:
                label mas_chess_save_multi_dlg:
                    m 1eua "Así que he estado pensando, [player]..."
                    m 1euc "La mayoría de la gente que se va en medio de una partida de ajedrez no vuelve para empezar una nueva."
                    m 3eud "...Así que no tiene sentido que lleve la cuenta de más de un juego inacabado entre nosotros."
                    m 1rka "Y como tenemos [game_count] juegos en progreso..."
                    m 3euc "Tengo que pedirte que elijas sólo uno para conservar.{w=0.2} Lo siento, [player]."
                    $ pick_text = _("Elige un juego que quieras conservar.")
                ...
                python:
                    ...
                    for pgn_game in actual_pgn_games:
                        ...
                        try:
                            ...
                            mas_utils.writelog("Fallo en la eliminación del juego: {0}\n".format(game_path))
...
label mas_chess_dlg_quicksave_lost:
    ...
    m 2lksdlb "Uh, [player]...{w=0.5} creo que me he equivocado al guardar la última partida y ahora ya no puedo abrirla."

    if qs_gone_count == 2:
        m 1lksdld "Lo siento mucho, mucho, [player]..."
        show monika 1ekc
        pause 1.0
        m 1eka "Pero no te preocupes, te compensaré...{w=0.3}{nw}"
        extend 3hua "¡iniciando una nueva partida!"
        m 3hub "Jajaja~"

    elif qs_gone_count == 3:
        m 1lksdlc "Soy tan tonta, [player]...{w=0.3} Lo siento."
        m 3eksdla "Empecemos un nuevo juego en su lugar."

    elif qs_gone_count % 5 == 0:
        m 2esc "Esto ya ha sucedido [qs_gone_count] veces hasta ahora..."
        m 2tsc "Me pregunto si esto es un efecto secundario de {i}alguien{/i} que trata de editar los guardados.{w=1}.{w=1}."
        m 7rsc "Como sea..."
        m 1esc "Empecemos un nuevo juego."

    else:
        m 1lksdlc "Lo siento..."
        m 3eka "Empecemos un nuevo juego en su lugar."
...
label mas_chess_dlg_quickfile_lost:
    ...
    m 2ekc "Juraría que teníamos una partida inacabada, pero no encuentro el archivo de guardado."

    m 2tkc "¿Te has metido con los guardados, [player]?{nw}"
    $ _history_list.pop()
    menu:
        m "¿Te has metido con los guardados, [player]?{fast}"
...
label mas_chess_dlg_quickfile_lost_deleted:
    ...
    menu:
        ...
        "Exacto.":
            ...
            m 1hua "Empecemos un nuevo juego~"

        "No.":
            m 1etc "¿Oh?"
            m 1rsc "Supongo que entonces lo has borrado por error."
            m 1eua "Empecemos un nuevo juego."
    return
...
label mas_chess_dlg_quickfile_lost_ofcoursenot:
    ...
    if qf_gone_count in [3,4]:
        ...
        m "..."
        m 1esc "Como sea.{w=0.5} Empecemos un nuevo juego."
...
label mas_chess_dlg_quickfile_lost_maybe:
    ...
    if qf_gone_count == 2:
        ...
        m "Ahora vamos a continuar nuestro juego."
...
label mas_chess_dlg_quickfile_lost_maybe_filechecker_no_file:
    m 1ekd "[player]..."
    m 1eka "No pasa nada. Vamos a jugar un nuevo juego."
    return None
    ...
    if qf_edit_count == 1:
        ...
        menu:
            "Lo siento.":
                ...
                m 1eua "Por suerte, aún recuerdo un poco del último partido, así que podemos continuar desde ahí."
                return store.mas_chess.CHESS_GAME_BACKUP
            ...
            "...":
                ...
                hide screen mas_background_timed_jump
                m 1lfc "Ya que ese juego ha sido arruinado, juguemos un nuevo juego."
...
label mas_chess_dlg_quickfile_edited_no:
    ...
    if qf_edit_count == 1:
        ...
        m 1dsc "Hmm..."
        m 1etc "El archivo de guardado se ve diferente de como lo recordaba por última vez,{w=0.2} {nw}"
        extend 1rksdlc "{nw}pero tal vez es sólo mi memoria que me falla..."
...
screen mas_chess_promote(q, r, n, b):
    ...
    frame:
        vbox:
            ...
            label _("Selecciona la pieza a promover"): # only use : if the choices are to the right of the label
                style "confirm_prompt"
                text_color mas_globals.button_text_idle_color
                xalign 0.5
...
init python:
    ...
    if mas_games.is_platform_good_for_chess():
        try:
            ...
            store.mas_utils.writelog("No se ha podido crear la carpeta del juego de ajedrez '{0}'\n".format(file_path))
            ...
            super(MASChessDisplayable, self).__init__(
                ...
                monika_move_quips={
                    "generic": [
                        ...
                        "Vale, entonces moveré esto aquí."
                    ],
                    "check": [
            ...
            def open_stockfish(path, startupinfo=None):
                ...
                #Catch the permission error
                    ...
                    if not renpy.windows:
                        ...
                        renpy.say(m, "Espera un segundo, [player]. Voy a intentar algo rápidamente.{w=0.3}.{w=0.3}.{w=0.3}{nw}")
        ...
        def check_buttons(self, ev, x, y, st):
            ...
            # inital check for buttons
                ...
                if self._button_save.event(ev, x, y, st):
                    wants_save = renpy.call_in_new_context("mas_chess_confirm_context", prompt=_("¿Quieres continuar más tarde?"))
                    if wants_save:
                    ...
                    wants_quit = renpy.call_in_new_context("mas_chess_confirm_context", prompt=_("¿Seguro que quieres rendirte?"))
                    if wants_quit:
