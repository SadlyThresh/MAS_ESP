...
label mas_showpoem(poem=None, paper=None, background_action_label=None):
    ...
    if background_action_label and renpy.has_label(background_action_label):
        call expression background_action_label from _call_expression_2
...
init 5 python:
    addEvent(
        Event(
            ...
            prompt="¿Puedo leer uno de tus poemas de nuevo?",
            category=["literatura"],
            pool=True,
            unlocked=True,
...
label monika_showpoem:
    ...
    python:
        ...
        renpy.say(m, "¿Cuál poema te gustaría leer?", interact=False)
    ...
    call mas_showpoem (_poem) from _call_mas_showpoem_5

    if not is_sad:
        m 3eka "Espero que te guste, [player]."

    m 1eka "¿Te gustaría leer otro poema?{nw}"
    $ _history_list.pop()
    menu:
        m "¿Te gustaría leer otro poema?{fast}"
        "Sí.":
