...
label mas_consumables_generic_get(consumable):
    ...
    python:
        ...
        if container:
            line_starter = renpy.substitute("Voy por un [mas_a_an_str(container)] de [consumable.disp_name].")


        elif obj_ref:
            line_starter = renpy.substitute("Voy por un [mas_a_an_str(obj_ref)] de [consumable.disp_name].")
    ...
    call mas_transition_from_emptydesk ("monika 1eua") from _call_mas_transition_from_emptydesk_22
...
label mas_consumables_generic_finish_having(consumable):
    ...
    if (not mas_canCheckActiveWindow() or mas_isFocused()) and not store.mas_globals.in_idle_mode:
        m 1eud "Terminé mi [consumable.disp_name].{w=0.2} {nw}"
...
label mas_consumables_generic_finished_prepping(consumable):
    ...
    if (not mas_canCheckActiveWindow() or mas_isFocused()) and not store.mas_globals.in_idle_mode:
        ...
        m 1esd "Oh, mi[plur] [consumable.disp_name][plur] [is_are] listo[plur]."
        m 1eua "Espera un momento."
    else:
    ...
    call mas_transition_from_emptydesk ("monika 1eua") from _call_mas_transition_from_emptydesk_24
    $ consumable.acs.keep_on_desk = True
...
label mas_consumables_generic_running_out(consumable):
    ...
    if amt_left > 0:
        ...
        m 3eud "Solo quería hacerte saber que solo tengo [amt_left] [line_ender]"
...
label mas_consumables_generic_queued_running_out_dlg(low_cons):
    ...
    if len(low_cons) > 2:
        ...
        m 3eua "Así que espero que no te importe, pero te dejé una lista de cosas en la carpeta de personajes."
        $ them = "lo"
    else:
