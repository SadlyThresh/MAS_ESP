...
label mas_o31_cleanup:
    ...
    with dissolve
    pause 1.0
    call mas_transition_from_emptydesk ("monika 1hua") from _call_mas_transition_from_emptydesk
    m 3hua "All done~"
    return
...
label greeting_o31_marisa:
    ...
    if store.mas_o31_event.cg_decoded:
        ...
        call spaceroom (hide_monika=True, scene_change=True) from _call_spaceroom




        call spaceroom (dissolve_all=True, scene_change=True, force_exp='monika 1eua_static') from _call_spaceroom_2
...
label greeting_o31_rin:
    ...
    if store.mas_o31_event.cg_decoded:
        ...
        scene black
        pause 1.0
        call spaceroom (scene_change=True, dissolve_all=True, force_exp='monika 1hksdlb_static') from _call_spaceroom_4
...
label greeting_o31_orcaramelo_hatsune_miku:
    if not persistent._mas_o31_relaunch:
        call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True) from _call_spaceroom_11
        ...
        call spaceroom (scene_change=True, dissolve_all=True) from _call_spaceroom_12
...
label greeting_o31_orcaramelo_sakuya_izayoi:
    call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True) from _call_spaceroom_13
    ...
    call mas_transition_from_emptydesk ("monika 3hub") from _call_mas_transition_from_emptydesk_9
...
label greeting_o31_generic:
    call spaceroom (scene_change=True, dissolve_all=True) from _call_spaceroom_14
...
label bye_trick_or_treat:
    ...
    if too_early_to_go:
        ...
        menu:
            ...
            "En realidad, {i}es{/i} un poco tarde":
...
label bye_trick_or_treat_wait_wait:
    ...
    menu:
        m "What is it?"
        "Tienes razón, es muy temprano." if too_early_to_go:
            call mas_dockstat_abort_gen from _call_mas_dockstat_abort_gen_1
            call mas_transition_from_emptydesk (exp="monika 3hub") from _call_mas_transition_from_emptydesk_10

            m 3hub "Ahaha, I told you!"
            m 1eka "Let's wait 'til evening, okay?"
            return True

        "Tienes razon, es muy tarde." if too_late_to_go:
            call mas_dockstat_abort_gen from _call_mas_dockstat_abort_gen_2

            if persistent._mas_o31_tt_count:
                call mas_transition_from_emptydesk (exp="monika 1hua") from _call_mas_transition_from_emptydesk_11
                m 1hua "Ahaha~"
                m "I told you."
                m 1eua "We'll have to wait until next year to go again."
            else:

                call mas_transition_from_emptydesk (exp="monika 2dkc") from _call_mas_transition_from_emptydesk_12
                m 2dkc "..."
                m 2ekc "Alright, [player]."
                m "It sucks that we couldn't go trick or treating this year."
                m 4eka "Let's just make sure we can next time, okay?"
            ...
            return True
        "Realmente, no puedo llevarte conmigo ahora.":

            call mas_dockstat_abort_gen from _call_mas_dockstat_abort_gen_3
            call mas_transition_from_emptydesk (exp="monika 1euc") from _call_mas_transition_from_emptydesk_13
...
label bye_trick_or_treat_rtg:
    ...
    if _return:
        call mas_transition_from_emptydesk (exp="monika 1hub") from _call_mas_transition_from_emptydesk_14
    ...
    call mas_transition_from_emptydesk (exp="monika 1ekc") from _call_mas_transition_from_emptydesk_15
    $ persistent._mas_o31_tt_count -= 1
    m 1ekc "Oh no..."
...
label mas_d25_gift_connector:
    python:
        d25_gift_quips = [
            _("¡Siguiente!"),
            _("¡Oh, hay otro más aquí!"),
            _("¡Ahora dejame abrir este de aqui!"),
            _("¡Este es el siguiente!")
        ]
...
label mas_d25_spent_time_monika:
    ...
    if mas_isMoniEnamored(higher=True):
        ...
        call mas_showpoem (poem_to_show, background_action_label="mas_d25_poem_mistletoe") from _call_mas_showpoem_2
...
label mas_d25_monika_christmas_eve:
    ...
    if (
        ...
        if mas_hasUnlockedClothesWithExprop("lingerie"):
            ...
            m 5ekbfa "..."
            show monika 1ekbfa zorder MAS_MONIKA_Z at t11 with dissolve_monika
            call mas_lingerie_intro (holiday_str="la Nochebuena", lingerie_choice=mas_clothes_santa_lingerie) from _call_mas_lingerie_intro
    ...
    if not mas_lastSeenLastYear("monika_resolutions"):
        ...
        menu:
            ...
            "Sí.":
                ...
                call monika_resolutions_accomplished_resolutions_menu ("¿Completaste tus metas del año pasado?") from _call_monika_resolutions_accomplished_resolutions_menu
            "No.":
        ...
        call monika_resolutions_accomplished_resolutions_menu ("Desde que hiciste tus metas del año pasado, ¿pudiste cumplirlas?") from _call_monika_resolutions_accomplished_resolutions_menu_1
...
label monika_nye_year_review:
    ...
    if not mas_HistWasFirstValueIn(True, datetime.date.today().year - 1, "pm.actions.monika.got_fresh_start"):
        ...
        if store.mas_anni.anniCount() > 0:
            $ ending = "fuera el mejor año que podría soñar"

            if mas_lastSeenLastYear("monika_nye_year_review"):
                $ ending = "fuera incluso mejor que el anterior"
...
label mas_nye_monika_nye_dress_intro:
    ...
    call mas_clothes_change (mas_clothes_dress_newyears, outfit_mode=True, unlock=True) from _call_mas_clothes_change_4
...
label greeting_pd25e_returned_nydp:
    ...
    if mas_isNYD():
        $ new_years = "la víspera de Año Nuevo"
...
label mas_player_bday_opendoor:
    ...
    call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True, show_emptydesk=False) from _call_spaceroom_16
    $ mas_disable_quit()
    if mas_isMonikaBirthday():
        $ your = "nuestra"
    else:
...
label mas_player_bday_surprise:
    $ persistent._mas_player_bday_decor = True
    call spaceroom (scene_change=True, dissolve_all=True, force_exp='monika 4hub_static') from _call_spaceroom_17
    m 4hub "Surprise!"
...
label mas_player_bday_opendoor_listened:
    ...
    $ persistent._mas_player_bday_opened_door = True
    $ persistent._mas_player_bday_decor = True
    call spaceroom (hide_monika=True, scene_change=True, show_emptydesk=False) from _call_spaceroom_18
    $ mas_disable_quit()
    if mas_isMonikaBirthday():
        $ your = "nuestra"
    else:
...
label mas_player_bday_card:
    ...
    call mas_showpoem (mas_poems.getRandomPoem("pbday")) from _call_mas_showpoem_3
...
label mas_monika_gets_cake:
    ...
    call mas_transition_from_emptydesk ("monika 6esa") from _call_mas_transition_from_emptydesk_16
...
label mas_f14_monika_valentines_intro:
    ...
    if not persistent._mas_f14_in_f14_mode or mas_canShowRisque():
        ...
        if mas_SELisUnlocked(mas_clothes_sundress_white) and mas_canShowRisque() and not mas_hasUnlockedClothesWithExprop("lingerie"):
            call mas_lingerie_intro (holiday_str="el Día de San Valentín", lingerie_choice=mas_clothes_vday_lingerie) from _call_mas_lingerie_intro_1
...
label mas_f14_monika_spent_time_with:
    ...
    if not mas_isBelowZero():
        ...
        if not poem_vday_1.is_seen():
            call mas_showpoem (poem_vday_1) from _call_mas_showpoem_4
            m "I really mean that, [player]..."
            m 3ekbsa "In you I found everything I could ever hope for~"

        elif not poem_vday_2.is_seen():
            call mas_showpoem (poem_vday_2) from _call_mas_showpoem_5
            m "You really are everything to me, [player]~"
        else:

            call mas_showpoem (mas_poems.getRandomPoem("f14")) from _call_mas_showpoem_6
...
label mas_HideCake(cake_type, reset_zoom=True):
    ...
    with dissolve
    $ renpy.pause(3.0, hard=True)
    call mas_transition_from_emptydesk ("monika 6esa") from _call_mas_transition_from_emptydesk_18
